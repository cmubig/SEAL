...
from metadrive.scenario.parse_object_state import parse_object_state, get_idm_route, get_max_valid_indicis, parse_full_trajectory
...
class ScenarioTrafficManager(BaseManager):
    ...
    def __init__(self):
        ...
        # Keep track of objects which have reached their destination already
        self._reached_destination = set()
        ...
        self.spawn_once = self.engine.global_config["spawn_once"]
        self.remove_after_destination = self.engine.global_config["remove_after_destination"]
    ...
    def before_step(self, *args, **kwargs):
        ...
        for v in self.spawned_objects.values():
            if self.engine.has_policy(v.id, ScenarioIDMPolicy):
                ...
                # TODO: track the obj_ids in a scenario which have *already* been spawned/complete
                if p.arrive_destination:
                    if self.remove_after_destination:
                        self._obj_to_clean_this_frame.append(self._obj_id_to_scenario_id[v.id])
                    self._reached_destination.add(self._obj_id_to_scenario_id[v.id])
                else:
                    do_speed_control = self.episode_step % self.IDM_ACT_BATCH_SIZE == p.policy_index
                    v.before_step(p.act(do_speed_control))
            elif not self.engine.has_policy(v.id, ReplayTrafficParticipantPolicy):
                p = self.engine.get_policy(v.name)
                v.before_step(p.act())
    ...
    def after_reset(self):
        ...
        self.idm_policy_count = 0
        self._reached_destination = set()
        # Initial spawning at timestep 0
    ...
    def after_step(self, *args, **kwargs):
        if self.episode_step < self.current_scenario_length:
            replay_done = False
            for scenario_id, track in self.current_traffic_data.items():
                # Possible additional spawning, if agent ended early or if only observed for a few timesteps
                prevent_spawn = self.spawn_once and scenario_id in self._reached_destination
                if not prevent_spawn and scenario_id not in self._scenario_id_to_obj_id:
                    if track["type"] == MetaDriveType.VEHICLE:
                        self.spawn_vehicle(scenario_id, track)
                    elif track["type"] == MetaDriveType.CYCLIST:
                        self.spawn_cyclist(scenario_id, track)
                ...
                elif not prevent_spawn and self.has_policy(self._scenario_id_to_obj_id[scenario_id], ReplayTrafficParticipantPolicy):
                ...
                if not prevent_spawn and track["type"] == MetaDriveType.VEHICLE and scenario_id in self._scenario_id_to_obj_id:
                    vname = self._scenario_id_to_obj_id[scenario_id]
                    self.v_map[vname].after_step()
        else:
            replay_done = True
    ...
    def set_adv_info(self,adv_name,adv_traj):
        ...
        # perhaps override map_manager.other_routes[adv_name]
        if len(adv_traj):
            self.engine.map_manager.other_routes[adv_name] = get_idm_route(adv_traj)
            if self.adv_name in self.engine.get_objects().keys():
                adv_obj = self.engine.get_object(self.adv_name).get(self.adv_name)
                adv_obj.navigation.set_route(None, None)
                adv_obj.navigation.update_localization(adv_obj)
    ...
    def spawn_vehicle(self, v_id, track):
        ...
        v_config["need_navigation"] = self.engine.global_config["traffic_need_navigation"]
        v_config.update(
            dict(
        ...
        if self.engine.global_config["ignore_idm_region"]:
            idm_ok = True
        ...
        if not need_reactive_traffic or v_id in self._static_car_id or not idm_ok or not length_ok:
            ...
            # TODO: set initial velocity, for reactive IDM background agents
