...
class ScenarioMapManager(BaseManager):
    ...
    def update_route(self):
        ...
        self.other_routes = {}
        for track_id, track in data[data.TRACKS].items():
            # Valid, instead of full, since some of these tracks may not be present entire time
            other_traj = parse_full_trajectory(track, valid_only=True, ignore_gaps=self.engine.global_config['ignore_gaps'])
            self.other_routes[track_id] = get_idm_route(other_traj)
