...
class DecoderResCat(nn.Module):
    ...
    def forward(self, hidden_states, ret_last=False):
        # Residual connection with (Relu(LayerNorm(Linear(x)))) cat (x)
...
class Decoder(nn.Module):
    ...
    def goals_2D_per_example(self, i: int, goals_2D: np.ndarray, mapping: List[Dict], lane_states_batch: List[Tensor],
        ...
        # This is where the score for each goal is actually set; i.e., these are the hidden states that matter?
    ...
    def goals_2D_eval(self, batch_size, mapping, labels, hidden_states, inputs, inputs_lengths, device):
        ...
        # This is all after the NMS stuff has been applied and such
        if 'tnt' in args.other_params or 'densetnt' in args.other_params:
            pred_trajs_batch = []
            pred_last_states_batch = []
            pred_yaws_batch = []
            for i in range(batch_size):
                ...
                if args.classify_sub_goals:
                    ...
                    predict_trajs, last_states = self.tnt_decoder(
                        torch.cat([hidden_states[i, 0, :].unsqueeze(0).expand(len(targets_feature), -1), targets_feature,
                                   hidden_attention], dim=-1), ret_last=True)
                    predict_trajs = predict_trajs.view([self.mode_num, self.future_frame_num, 2])
                    predict_trajs = np.array(predict_trajs.tolist())
                    last_states = np.array(last_states.tolist())
                else:
                    # TODO: return state here?
                    # Perhaps what we want is the last dimension of goals_decoder
                    # Final input dimension is 32 x 384
                    # Decoder is just an MLP with a linear unit, layer_norm, and a final linear unit out
                    # last_states is 32 x 512
                    predict_trajs, last_states = self.tnt_decoder(
                        torch.cat([hidden_states[i, 0, :].unsqueeze(0).expand(len(targets_feature), -1), targets_feature,
                                   hidden_attention], dim=-1), ret_last=True)
                    predict_trajs = predict_trajs.view([self.mode_num, self.future_frame_num, 2])
                ...
                pred_last_states_batch.append(last_states)
        ...
        if 'pred_yaw' in args.other_params:
            return pred_trajs_batch, pred_probs_batch, pred_yaws_batch, pred_indices_batch, pred_last_states_batch
        else:
            return pred_trajs_batch, pred_probs_batch, pred_indices_batch, pred_last_states_batch
    ...
    def forward(self, mapping: List[Dict], batch_size, lane_states_batch: List[Tensor], inputs: Tensor,
        ...
        if 'variety_loss' in args.other_params:
            ...
            # By default, this is the only path enabled
            for i in range(batch_size):
                goals_2D = mapping[i]['goals_2D']
            ...
            # By default, this is False, so skip this path
            if 'tnt_square' in args.other_params:
            ...
            # This is also by default True, it seems...
            if args.do_eval:
