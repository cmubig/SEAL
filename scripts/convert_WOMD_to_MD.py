...
def parse_data(file_list, input_path, output_path, filter_path = None, worker_index=None):
    ...
    if filter_path is not None:
        ...
        filter_ids = set([x['scenario_id'] for x in filter_info])
        assert len(filter_ids) == len(filter_ids), 'Duplicate scene ids'
        filter_info = {x['scenario_id']: x for x in filter_info}
    else:
        filter_info = None
        filter_ids = set()
    ...
    for file_count, file in tqdm(enumerate(file_list), 'Searching shards', total=len(file_list)):
        ...
        for j, data in enumerate(dataset.as_numpy_iterator()):
            ...
            if filter_info is not None and not scenario.scenario_id in filter_ids:
                continue
            extra_info = None if filter_info is None else filter_info[scenario.scenario_id]
            ...
            # Overwrite with saved info from select_scored_cases
            if extra_info is not None:
                scenario.sdc_track_index = extra_info['sdc_track_index']
            ...
            if extra_info is not None:
                md_scenario['sdc_score'] = extra_info['sdc_score']
                md_scenario['traj_scores_asym_combined'] = extra_info['traj_scores_asym_combined']
                md_scenario['traj_scores_fe'] = extra_info['traj_scores_fe']
...
if __name__ == "__main__":
    ...
    parser.add_argument('--do_filter', action='store_true', help='Whether to use filter')
    ...
    if args.do_filter:
        train_dir = raw_data_path.replace('testing', 'training')
        val_dir = raw_data_path.replace('testing', 'validation')
        train_list = [os.path.join(train_dir, x) for x in os.listdir(train_dir)]
        val_list = [os.path.join(val_dir, x) for x in os.listdir(val_dir)]
        file_list = train_list + val_list
        raw_data_path = ''
    parse_data(file_list, raw_data_path, output_path, filter_path=None if not args.do_filter else args.filter_path)
    sys.exit()
