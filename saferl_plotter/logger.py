...
class SafeLogger():
    def __init__(self, log_dir="./logs", exp_name=None, env_name=None, seed=0,config=None, filename="logger.csv", fieldnames=[], debug=False, allow_overwrite=False):
        self.exp_name = exp_name
        self.env_name = env_name
        self.seed = seed
        self.previous_log_time = time.time()
        ...
        if debug:
            ...
            self.log_dir = f"./{log_dir}/{exp_name.replace('-', '_')}_{env_name.replace('-', '_')}-seed{seed}"
            while True:
                if not allow_overwrite and os.path.exists(f"{self.log_dir}-{str(num_exps)}/"):
                    num_exps += 1
                else:
                    self.log_dir += f"-{str(num_exps)}/"
                    os.makedirs(self.log_dir, exist_ok=allow_overwrite)
                    break     
            self.csv_file = open(self.log_dir + '/' + filename, 'w', encoding='utf8')
            # header={"t_start": time.time(), 'env_id' : env_name, 'exp_name': exp_name, 'seed': seed}
            # header = '# {} \n'.format(json.dumps(header))
